...
\end{tabbing}
  Here \text{\ttfamily modify} takes an update action \text{\ttfamily f}\footnote{\text{\ttfamily f} returns a list rather
  than a single element to prevent curious readers from suggesting to use a
  boxed array instead of a list.}, an index \text{\ttfamily i}, and a list \text{\ttfamily xs} and returns a
  new list with the \text{\ttfamily i}'th element replaced with the result of \text{\ttfamily f}.%
...
\texfamily ~~~~{\bfseries deriving}~({\itshape Show},~{\itshape Eq})
\end{tabbing}
  where the \text{\ttfamily ListContext} keeps track of elements to the left and to the right
...
\end{tabbing}
  ...
  Finally, we define a local version of our \text{\ttfamily modify} function (\TODO{Boy, is
  this function ugly...})
\begin{tabbing}\texfamily
...
\end{tabbing}
  ...
  bloat\footnote{%
  Operator \text{\ttfamily \char62{}\char61{}\char62{}} comes from \text{\ttfamily Control\char46{}Monad} module in \text{\ttfamily base}
...
\end{tabbing}
  \text{\ttfamily TreeContext} stores everything we need to reconstruct the hole, and \text{\ttfamily tzUp} does
  exactly that:
\begin{tabbing}\texfamily
...
\end{tabbing}
  Implementation of \text{\ttfamily tzLeft}, \text{\ttfamily tzRight}, and \text{\ttfamily tzEnter} is very similar to the
  List zipper case and is left as an exercise for the reader. \text{\ttfamily tzLeave} differs
...
\end{tabbing}
  ...
  the constructor and its arguments. Even though \text{\ttfamily Data\char46{}Data\char46{}gfold} allows us to achieve
...
\end{tabbing}
  For example, here is how we can make \text{\ttfamily Tree} an instance of \text{\ttfamily Disssectible}
\begin{tabbing}\texfamily
...
\end{tabbing}
  We can unpack a \text{\ttfamily Fork} and the zipper will thus be able to go down. \text{\ttfamily Leaf}s,
  however, are left untouched and trying to go down from a \text{\ttfamily Leaf} will return
  \text{\ttfamily Nothing}.
...
\end{tabbing}
  For example, for a tuple \text{\ttfamily \char40{}Int\char44{}~Int\char44{}~Int\char44{}~Int\char44{}~Int\char44{}~Int\char41{}}, we can have
\begin{tabbing}\texfamily
...
\end{tabbing}
  And just like before the \text{\ttfamily Zipper} is a product of the hole and context:
\begin{tabbing}\texfamily
