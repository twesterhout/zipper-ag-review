...
\end{hscode}\resethooks
  ...
  bloat\footnote{%
  Operator \mimplies comes from \text{\tt Control\char46{}Monad} module in \text{\tt base}
  and has the following signature:
\begin{hscode}\SaveRestoreHook
~~(\mimplies)~::~{\itshape Monad}~m~=>~(a~\WithMath{->}{\rightarrow}~m~b)~\WithMath{->}{\rightarrow}~(b~\WithMath{->}{\rightarrow}~m~c)~\WithMath{->}{\rightarrow}~a~\WithMath{->}{\rightarrow}~m~c\ColumnHook
\end{hscode}\resethooks
  }:
\begin{hscode}\SaveRestoreHook
~~modifyExample~::~{\itshape IO}~()\\
\texfamily ~~modifyExample~=~print~\$\\
\texfamily ~~~~lzEnter~@{\itshape Int}~\mimplies~right\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~right\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~lzModify~(const~[])\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~lzModify~(return~\WithMath{.}{\circ}~(+1))\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~left\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~lzModify~(return~\WithMath{.}{\circ}~negate)\\
\texfamily ~~~~~~~~~~~~~~~~~\mimplies~return~\WithMath{.}{\circ}~lzLeave~\$\\
\texfamily ~~~~~~[1,~2,~3,~4,~5]\ColumnHook
\end{hscode}\resethooks
...
\texfamily ~~~~go~({\itshape Fork}~xs~ys~~)~m~=~({\itshape Fork}~xs'~ys',~min~m\char95 x~m\char95 y)\\
\texfamily ~~~~~{\bfseries where}\\
\texfamily ~~~~~~(xs',~m\char95 x)~=~go~xs~m\\
\texfamily ~~~~~~(ys',~m\char95 y)~=~go~ys~m\ColumnHook
\end{hscode}\resethooks

\begin{comment}
  Alternatively, we can formulate repmin in terms of an attribute grammar. We
  use \text{\tt localMin} attribute to represent local minimum of a subtree, \text{\tt globalMin}
  to represent the global minimum, and \text{\tt updated} for the solution for a subtree.
\end{comment}
  ...
  \text{\tt Tree~Int} datatype from \TODO{where?}.
...
\end{hscode}\resethooks
  This component declares, using the \text{\tt SYN} keyword, that elements of type \text{\tt Tree}
  synthesize an attribute \text{\tt localMin} of type \text{\tt Int}. Then, a \text{\tt SEM} sentence
  defines how \text{\tt localMin} is computed: when the current tree is a leaf, clearly
  its minimal value is the leaf value itself; when it is the fork of two other
  trees (the \text{\tt left} and the \text{\tt right} subtrees), we compute the minimal values of
  each subtree (i.e., their \text{\tt localMin} attribute), and then their minimal value
  (function \text{\tt min}). In this notation, \text{\tt lhs} refers to the left-hand side symbol
