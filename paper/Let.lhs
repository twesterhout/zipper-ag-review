...
excerpts. In particular, we will focus on |let| expressions that are common in
...
The values associated with |program1| and |program2| are, quite
straightforwardly, |4| and |7|, respectively. As of |program3|, its value of
|10| is calculated adding the value of |x|, which is |4| and the value of |y|,
i.e., |6|. The value of |y| is obtained by adding the value of |w|, which is
|2|, and the value of |x|, which, again, is |4|.
...
e.g., that the semantic meaning of processing |programe| is |[w, x]|: the use
of the undeclared variable |w| occurs in line 3 whereas the duplicate
declaration of |x| occurs in line 4.
...
Notice that |duplicateLet| not only computes the total environment (using an
initially empty accumulating parameter), but it also computes a |Let2|
...
|missingLet| giving that computed data structure and the accumulated environment
as arguments. It produces the list of errors that follows the sequential
structure of the program.

In function |duplicateDecls|, for every block we compute: its environment, its
...
function |missingDecls| for the constructor |Nested2|, the program performs the
two traversals to the body of that expression: calls |duplicateLet| and
|missingLet|.
